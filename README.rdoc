...
- When a request comes in with the format http://our-site/fEb17, redirect user to the original url
	for e.g,
		...
		=> redirect to the original_url returned, 
			if no matches found, show index page
...
- routes
	...
	#index would map to #new for convenience	
	#show would retrieve the original url and redirect the user there automatically
